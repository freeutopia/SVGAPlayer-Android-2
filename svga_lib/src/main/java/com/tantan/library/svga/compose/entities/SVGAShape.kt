...
import com.tantan.library.svga.compose.proto.ShapeEntity
...
class SVGAVideoShapeEntity(obj: ShapeEntity) {
  ...
  private fun checkValueRange(color: ShapeEntity.ShapeStyle.RGBAColor) = when {
    (color.r ?: 0f) + (color.g ?: 0f) + (color.b ?: 0f) <= 3f -> 255f
    else -> 1f
  }

  // 检查 alpha 范围是否是 [0f, 1f]，有可能是 [0f, 255f]
  private fun checkAlphaValueRange(color: ShapeEntity.ShapeStyle.RGBAColor) = when {
    color.a <= 1f -> 255f
    else -> 1f
  }
